...
router.post('/read', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Call the service function
    const requisites = await requisiteController.readRequisite(code1, code2, type);
    ...
    // Handle errors from the service
    if (error instanceof Error && error.message === 'The course combination or type provided does not exist.') {
      res.status(HTTP.FORBIDDEN).json({ error: error.message });
    } else {
